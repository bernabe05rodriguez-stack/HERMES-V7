...
class Hermes:
    ...
    def _send_audio_note(self, ui_device, device_id, whatsapp_package):
        ...
        child_button_selectors = [
            dict(className="android.widget.ImageButton"),
            dict(classNameMatches="(?i).*imagebutton"),
            dict(descriptionMatches="(?i).*nota de voz.*"),
            dict(descriptionMatches="(?i).*microf.*"),
            dict(descriptionMatches="(?i).*audio.*"),
            dict(resourceIdMatches="(?i).*voice.*"),
            ...
            try:
                ...
                if ('button' not in class_name and not info.get('clickable')) or class_name in {"android.view.view"}:
                    for child_selector in child_button_selectors:
                        try:
                            child = candidate.child(**child_selector)
                            if child.exists:
                                child_info = child.info or {}
                                child_bounds = child_info.get('bounds') or {}
                                if child_bounds and child_bounds.get('left') != child_bounds.get('right'):
                                    candidate = child
                                    info = child_info
                                    class_name = (info.get('className') or '').lower()
                                    bounds = child_bounds
        ...
        if not center:
            try:
                ...
                bounds = info.get('bounds') or mic_bounds or {}
                if not bounds:
        ...
        if not center:
            ...
            if width == 0 or height == 0:
                self.log("Coordenadas inválidas para el botón de audio.", 'warning')
                return False

            if width > height * 1.5:
                x = right - max(10, int(height * 0.45))
            else:
                x = left + width // 2
            y = top + height // 2

            mic_bounds = dict(left=left, right=right, top=top, bottom=bottom)
        elif not mic_bounds:
            try:
                info = mic_button.info or {}
                mic_bounds = info.get('bounds')
            except Exception:
                mic_bounds = None

        if mic_bounds:
            width = max(0, int(mic_bounds['right']) - int(mic_bounds['left']))
            height = max(0, int(mic_bounds['bottom']) - int(mic_bounds['top']))
